...
TimeMap.prototype.set = function(key, value, timestamp) {
  this.map.set(timestamp, [key, value]);
...
TimeMap.prototype.get = function(key, timestamp) {
  if (this.arr[0] > timestamp) return '';
  const idx = this.binarySearch(timestamp);
  const [k, v] = this.map.get(this.arr[idx]);
  if (k === key) return v;
  if (idx !== 0) return this.get(key, this.arr[idx] - 1);
  return '';
};
...
/**
 * 也可以考虑存储数据结构：Map(key, [[val1, timestamp1], [val2, timestamp2]])
 */